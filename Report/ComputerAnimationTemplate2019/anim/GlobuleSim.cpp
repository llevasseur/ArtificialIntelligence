...
double GlobuleSim::Cr(double r0)
{
	...
	{
		//animTcl::OutputMessage("Cr is r0 because powder or liquid.");
		return r0;
	}
} // GlobuleSim::Cr
...
double GlobuleSim::Cd(double r0)
{
	...
	{
		//animTcl::OutputMessage("REACHED!");
		//animTcl::OutputMessage("Cd is r0 because powder.");
		return r0;
	}
...
void GlobuleSim::solveF(int index, vector<double*> globs, vector<double*> vel, vector<double> radii, double r0, double mass, double* FTotal)
{
	...
	for (int i = 0; i < globs.size(); i++)
	{
		...
		if (i != index) // Maybe also check if r >= rmax?
		{
			Fip[0] = 0.0f;
			Fip[1] = 0.0f;
			Fip[2] = 0.0f;
			...
			// currentGlob = globs[index]
			// currentVel = vel[index]
			// otherGlob = globs[i];
			//otherVel = vel[i];
			// otherRadius = radii[i]
			//animTcl::OutputMessage("inForce, globs[index][1]: %f, globs[i][1]: %f", globs[index][1], globs[i][1]);
			...
			else
			{
				...
				//animTcl::OutputMessage("vBar: %f %f %f", vBar[0], vBar[1], vBar[2]);
			...
			//animTcl::OutputMessage("D: %f ||pBar: %f %f %f|| DragTerm: %f, RTerm: %f", D, pBar[0], pBar[1], pBar[2], dragTerm, repulsionTerm, Fip[1]);
