...
void MakeScene(void)
{
	...
	*/
	...
	Globules* globSys = new Globules("globSys");

	success = GlobalResourceManager::use()->addSystem(globSys, true);
	...
	GlobuleSim* globSim =
		new GlobuleSim("globSim", globSys);

	success = GlobalResourceManager::use()->addSimulator(globSim);
